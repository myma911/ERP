...
public class LoginService {
	...
	public R2 login(LoginForm form) {
		 return login20200814(form);
	}

	private R2 login20200814(LoginForm form) {
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes())
				.getRequest();
		HttpSession session = request.getSession(false);
		if (null == session) {
			return new R2(StateCodeEnum.VERIFICATION_CODE_ERROR);
		}
		...
		if (user == null) {
			return new R2(StateCodeEnum.LOGIN_ERROR);
		}

		// 获取登录token
		TokenEntity tokenEntity = tokenService.createToken(user.getUserId());

		return R2.OK(tokenEntity.getToken());
	}
